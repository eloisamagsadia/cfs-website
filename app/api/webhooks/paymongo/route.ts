...
export async function POST(req: NextRequest) {
  ...
  if (eventType === "payment.paid") {
    ...
    if (type === "order" && reference) {
      const { data: order } = await supabase.from("orders").update({ payment_status:"paid", paymongo_ref:eventData.id, order_status:"processing" }).eq("id", reference).select("*, profiles(*)").single();
      if (order) {
        try {
    ...
    if (type === "registration" && reference) {
      const { data: reg } = await supabase.from("event_registrations").update({ payment_status:"paid", paymongo_ref:eventData.id }).eq("id", reference).select("*, events(*), profiles(*)").single();
      if (reg) {
        try {
    ...
    if (type === "donation" && reference) {
      const { data: donation } = await supabase.from("donations").update({ status:"completed", paymongo_ref:eventData.id }).eq("id", reference).select("*, profiles(*)").single();
      if (donation) {
        try {
  ...
  if (eventType === "payment.failed") {
    const { reference, type } = eventData.attributes?.metadata ?? {};
    if (type === "order") await supabase.from("orders").update({ payment_status:"failed" }).eq("id", reference);
    if (type === "registration") await supabase.from("event_registrations").update({ payment_status:"failed" }).eq("id", reference);
    if (type === "donation") await supabase.from("donations").update({ status:"failed" }).eq("id", reference);
  }

...
export async function POST(req: NextRequest) {
  ...
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  await logAudit({ userId, action: "create_event", target_type: "event", target_id: (data as any)?.id, details: { title: title?.trim() }, req });
...
export async function PATCH(req: NextRequest) {
  ...
  if (updates.guidelines_url !== undefined) payload.guidelines_url = updates.guidelines_url?.trim() || null;
  ...
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  await logAudit({ userId, action: "edit_event", target_type: "event", target_id: id, details: { fields: Object.keys(payload) }, req });
...
export async function DELETE(req: NextRequest) {
  ...
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  await logAudit({ userId, action: "delete_event", target_type: "event", target_id: id, req });

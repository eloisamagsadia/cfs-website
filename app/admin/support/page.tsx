...
export default function AdminSupportPage() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      <div>
        <h1 style={{ fontFamily: R, fontSize: "1.6rem", color: "#1B3A2D", letterSpacing: "3px", marginBottom: "4px" }}>SUPPORT TICKETS</h1>
        <p style={{ fontFamily: B, fontSize: "13px", color: "#4A7C59" }}>{counts.open} open · {tickets.length} total</p>
      </div>

      {/* Filters */}
      <div style={{ background: "#FFFFFF", border: "2px solid #DDE8DD", borderRadius: "12px", padding: "16px 20px", display: "flex", flexDirection: "column", gap: "12px" }}>
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by subject, member, or category..."
          style={{ width: "100%", background: "#F2F7F2", border: "1.5px solid #DDE8DD", borderRadius: "6px", padding: "8px 14px", color: "#1B3A2D", fontFamily: B, fontSize: "13px", outline: "none", boxSizing: "border-box" }} />
      </div>
      ...
      {loading ? (
        ...
        <div style={{ background: "#FFFFFF", border: "2px solid #DDE8DD", borderRadius: "12px", overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            {filtered.map(t => (
              <div key={t.id}>
                ...
                <div onClick={() => setExpanded(expanded === t.id ? null : t.id)}
                  ...
                  <span style={{ fontFamily: B, fontSize: "12px", color: "#5A7A60" }}>{t.category}</span>
                  <span style={{ fontFamily: B, fontSize: "11px", color: "#5A7A60" }}>{timeAgo(t.created_at)}</span>
                  ...
                  <span style={{ color: "#5A7A60", fontSize: "12px", textAlign: "center" }}>{expanded === t.id ? "▲" : "▼"}</span>
                </div>
                ...
                {expanded === t.id && (
                  <div style={{ borderBottom: "1px solid #DDE8DD", padding: "20px", background: "#162010", display: "flex", flexDirection: "column", gap: "14px" }}>
                    <div style={{ background: "#F2F7F2", borderRadius: "8px", padding: "12px 14px" }}>
                      ...
                      <div style={{ fontFamily: B, fontSize: "13px", color: "#1B3A2D", lineHeight: 1.7 }}>{t.message}</div>
                    </div>
                        ...
                        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                          {t.attachments.map((url: string, i: number) => (
                            <img key={i} src={url} alt="" onClick={() => setLightboxImg(url)}
                              style={{ width: "100px", height: "100px", objectFit: "cover", borderRadius: "8px", border: "1.5px solid #DDE8DD", cursor: "zoom-in" }} />
                          ))}
                        </div>
                      </div>
                    )}
                    ...
                    <div style={{ borderTop: "1px solid #DDE8DD", paddingTop: "14px", display: "flex", flexDirection: "column", gap: "12px" }}>
                      <div style={{ display: "flex", gap: "16px", flexWrap: "wrap" }}>
                        <div style={{ flex: 1, minWidth: "200px" }}>
                          <div style={{ fontFamily: B, fontSize: "11px", color: "#5A7A60", marginBottom: "6px", letterSpacing: "1px" }}>STATUS</div>
                          <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                            {STATUSES.map(s => (
                              <button key={s} onClick={() => setStatuses(prev => ({ ...prev, [t.id]: s }))}
                                style={{ fontFamily: R, fontSize: "10px", padding: "4px 10px", borderRadius: "20px", border: `1.5px solid ${statuses[t.id] === s ? STATUS_COLORS[s] : "#DDE8DD"}`, background: statuses[t.id] === s ? STATUS_COLORS[s] + "20" : "transparent", color: statuses[t.id] === s ? STATUS_COLORS[s] : "#5A7A60", cursor: "pointer", letterSpacing: "1px" }}>
                        ...
                        <div style={{ flex: 1, minWidth: "200px" }}>
                          <div style={{ fontFamily: B, fontSize: "11px", color: "#5A7A60", marginBottom: "6px", letterSpacing: "1px" }}>PRIORITY</div>
                          <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                            {PRIORITIES.map(p => (
                              <button key={p} onClick={() => setPriorities(prev => ({ ...prev, [t.id]: p }))}
                                style={{ fontFamily: R, fontSize: "10px", padding: "4px 10px", borderRadius: "20px", border: `1.5px solid ${priorities[t.id] === p ? PRIORITY_COLORS[p] : "#DDE8DD"}`, background: priorities[t.id] === p ? PRIORITY_COLORS[p] + "20" : "transparent", color: priorities[t.id] === p ? PRIORITY_COLORS[p] : "#5A7A60", cursor: "pointer", letterSpacing: "1px" }}>
                                {p.toUpperCase()}
                              </button>
                            ))}
                          </div>
                      ...
                      <div>
                        <div style={{ fontFamily: B, fontSize: "11px", color: "#5A7A60", marginBottom: "6px", letterSpacing: "1px" }}>REPLY TO MEMBER</div>
                        <textarea value={adminNotes[t.id] ?? ""} onChange={e => setAdminNotes(prev => ({ ...prev, [t.id]: e.target.value }))}
                          placeholder="Write a reply..." rows={3}
                          style={{ width: "100%", background: "#F2F7F2", border: "1.5px solid #DDE8DD", borderRadius: "6px", padding: "10px 14px", color: "#1B3A2D", fontFamily: B, fontSize: "13px", outline: "none", resize: "vertical", boxSizing: "border-box" }} />
                      </div>
            ...
            {filtered.length === 0 && (
              <div style={{ padding: "48px", textAlign: "center", fontFamily: R, color: "#5A7A60", letterSpacing: "2px" }}>NO TICKETS</div>
            )}
          </div>
        </div>
      )}

        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              {[1, 2, 3, 4].map(q => (
                <button key={q} onClick={() => set("quarter", String(q))} style={{ flex: 1, fontFamily: R, fontSize: "13px", padding: "10px 0", borderRadius: "8px", border: `2px solid ${form.quarter === String(q) ? "#1A8040" : "#DDE8DD"}`, background: form.quarter === String(q) ? "#E8F0E4" : "#F2F7F2", color: form.quarter === String(q) ? "#1A8040" : "#5A7A60", cursor: "pointer", letterSpacing: "1px" }}>Q{q}</button>
              ))}
            </div>
          </div>
        </div>

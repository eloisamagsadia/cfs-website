  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      <div>
        ...
        <p style={{ fontFamily: B, fontSize: "13px", color: "#8AAA78" }}>Broadcast messages, send reminders, and track engagement</p>
      </div>

      {/* Stats */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(140px,1fr))", gap: "10px" }}>
        {[
          { label: "TOTAL SENT",   value: loadingStats ? "..." : (stats?.total ?? 0),       color: "#F0EAD6", bg: "#1A2614" },
          { label: "TOTAL READ",   value: loadingStats ? "..." : (stats?.read ?? 0),        color: "#3CCE2A", bg: "#1A3D14" },
          ...
          <div key={label} style={{ position: "relative", padding: "4px 4px 6px 0" }}>
            ...
            <div style={{ position: "relative", background: bg, border: "2px solid #2C4820", borderRadius: "10px", padding: "16px", zIndex: 1 }}>
      ...
      {stats?.byType?.length > 0 && (
        <div style={{ background: "#1A2614", border: "2px solid #2C4820", borderRadius: "12px", padding: "20px" }}>
          <div style={{ fontFamily: R, fontSize: "12px", color: "#F0EAD6", letterSpacing: "2px", marginBottom: "14px" }}>ENGAGEMENT BY TYPE</div>
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            {stats.byType.map((t: any) => (
              <div key={t.type} style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                ...
                <span style={{ fontFamily: R, fontSize: "11px", color: "#8AAA78", letterSpacing: "1px", width: "140px", flexShrink: 0, textTransform: "uppercase" }}>
                  {t.type.replace(/_/g, " ")}
                </span>
                <div style={{ flex: 1, height: "8px", background: "#243520", borderRadius: "20px", overflow: "hidden" }}>
                ...
                <span style={{ fontFamily: B, fontSize: "11px", color: "#5A7A50", flexShrink: 0 }}>{t.total}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ background: "#1A2614", border: "2px solid #2C4820", borderRadius: "12px", padding: "24px" }}>
        ...
        <div style={{ marginBottom: "18px" }}>
          <div style={{ fontFamily: B, fontSize: "11px", color: "#5A7A50", letterSpacing: "1px", marginBottom: "8px", textTransform: "uppercase" }}>Send To</div>
          <div style={{ display: "flex", gap: "8px" }}>
            {[
              ...
              <button key={v} onClick={() => setTarget(v as any)} style={{
                ...
                color: target === v ? "#F07228" : "#5A7A50",
                border: `1.5px solid ${target === v ? "#F07228" : "#2C4820"}`,
        ...
        {target === "member" && (
          <div style={{ marginBottom: "14px" }}>
            <div style={{ fontFamily: B, fontSize: "11px", color: "#5A7A50", letterSpacing: "1px", marginBottom: "6px", textTransform: "uppercase" }}>Search Member</div>
            <input value={memberSearch} onChange={e => setMemberSearch(e.target.value)} placeholder="Search by name..." style={inputStyle}/>
            {memberSearch && (
              <div style={{ background: "#243520", border: "1.5px solid #2C4820", borderRadius: "6px", marginTop: "4px", maxHeight: "160px", overflowY: "auto" }}>
                {filteredMembers.slice(0, 8).map(m => (
                  <div key={m.id} onClick={() => { upd("targetUserId", m.id); setMemberSearch(m.display_name ?? ""); }} style={{
                    ...
                    color: form.targetUserId === m.id ? "#3CCE2A" : "#F0EAD6",
                    background: form.targetUserId === m.id ? "#1A3D14" : "transparent",
                    borderBottom: "1px solid #2C4820",
                  }}>
                    {m.display_name ?? "Member"}
                  </div>
                ))}
                {filteredMembers.length === 0 && (
                  <div style={{ padding: "10px 14px", fontFamily: B, fontSize: "13px", color: "#5A7A50" }}>No members found</div>
                )}
              </div>
            )}
          </div>
        ...
        {target === "event" && (
          <div style={{ marginBottom: "14px" }}>
            <div style={{ fontFamily: B, fontSize: "11px", color: "#5A7A50", letterSpacing: "1px", marginBottom: "6px", textTransform: "uppercase" }}>Select Event</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px", marginBottom: "14px" }}>
          <div style={{ gridColumn: "1/-1" }}>
            <div style={{ fontFamily: B, fontSize: "11px", color: "#5A7A50", letterSpacing: "1px", marginBottom: "6px", textTransform: "uppercase" }}>Title *</div>
            <input value={form.title} onChange={e => upd("title", e.target.value)} placeholder="Notification title..." style={inputStyle} maxLength={100}/>
          </div>
          <div style={{ gridColumn: "1/-1" }}>
            <div style={{ fontFamily: B, fontSize: "11px", color: "#5A7A50", letterSpacing: "1px", marginBottom: "6px", textTransform: "uppercase" }}>Message *</div>
          ...
          <div>
            <div style={{ fontFamily: B, fontSize: "11px", color: "#5A7A50", letterSpacing: "1px", marginBottom: "6px", textTransform: "uppercase" }}>Type</div>
          ...
          <div>
            <div style={{ fontFamily: B, fontSize: "11px", color: "#5A7A50", letterSpacing: "1px", marginBottom: "6px", textTransform: "uppercase" }}>Link (optional)</div>
            <input value={form.link} onChange={e => upd("link", e.target.value)} placeholder="/members/events" style={inputStyle}/>
          </div>
        </div>

        {result && (
          <div style={{ background: result.ok ? "#1A3D14" : "#3D0A18", border: `1.5px solid ${result.ok ? "#3CCE2A" : "#F04060"}`, borderRadius: "8px", padding: "12px 16px", fontFamily: R, fontSize: "13px", color: result.ok ? "#3CCE2A" : "#F04060", letterSpacing: "1px", marginBottom: "14px" }}>
            {result.msg}
          </div>
        )}
        ...
        <button onClick={handleSend} disabled={sending} style={{ position: "relative", display: "block", background: "transparent", border: "none", padding: 0, cursor: sending ? "not-allowed" : "pointer", width: "100%" }}>
          ...
          <span style={{ position: "relative", display: "block", fontFamily: R, fontSize: "14px", background: sending ? "#1A3D14" : "#F07228", color: sending ? "#5A7A50" : "#F0EAD6", padding: "12px", border: "2px solid #080F06", borderRadius: "6px", textAlign: "center", letterSpacing: "2px" }}>
      ...
      {stats?.recent?.length > 0 && (
        <div style={{ background: "#1A2614", border: "2px solid #2C4820", borderRadius: "12px", padding: "20px" }}>
          <div style={{ fontFamily: R, fontSize: "12px", color: "#F0EAD6", letterSpacing: "2px", marginBottom: "14px" }}>RECENT NOTIFICATIONS</div>
          <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
            {stats.recent.map((n: any) => (
              <div key={n.id} style={{ display: "flex", gap: "10px", alignItems: "center", padding: "10px 0", borderBottom: "1px solid #2C4820" }}>
                <span style={{ fontSize: "14px", flexShrink: 0 }}>{TYPE_ICONS[n.type] ?? "🔔"}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontFamily: R, fontSize: "12px", color: "#F0EAD6", letterSpacing: "0.5px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{n.title}</div>
                  <div style={{ fontFamily: B, fontSize: "11px", color: "#5A7A50", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{n.message}</div>
                </div>
                <div style={{ display: "flex", gap: "8px", alignItems: "center", flexShrink: 0 }}>
                  <span style={{ fontFamily: R, fontSize: "10px", color: n.is_read ? "#3CCE2A" : "#F5C82A", letterSpacing: "1px" }}>
                    {n.is_read ? "READ" : "UNREAD"}
                  </span>
                  <span style={{ fontFamily: B, fontSize: "11px", color: "#5A7A50" }}>{timeAgo(n.created_at)}</span>
                </div>
              </div>
            ))}
          </div>

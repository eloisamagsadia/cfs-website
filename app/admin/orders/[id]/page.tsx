  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "24px", maxWidth: "800px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
        <div>
          <h1 style={{ fontFamily: R, fontSize: "1.6rem", color: "#1B3A2D", letterSpacing: "3px", marginBottom: "4px" }}>
            ORDER #{order.id.slice(0, 8).toUpperCase()}
          </h1>
          <p style={{ fontFamily: B, fontSize: "13px", color: "#5A7A60" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
        ...
        <div style={{ background: "#FFFFFF", border: "2px solid #DDE8DD", borderRadius: "12px", padding: "20px" }}>
          <div style={{ fontFamily: R, fontSize: "11px", color: "#4A7C59", letterSpacing: "2px", marginBottom: "12px" }}>CUSTOMER</div>
          <div style={{ fontFamily: R, fontSize: "14px", color: "#1B3A2D", marginBottom: "4px" }}>{order.profiles?.display_name ?? "Member"}</div>
          {addr && (
            <div style={{ fontFamily: B, fontSize: "12px", color: "#5A7A60", lineHeight: 1.6 }}>
        ...
        {/* Order summary */}
        <div style={{ background: "#FFFFFF", border: "2px solid #DDE8DD", borderRadius: "12px", padding: "20px" }}>
          <div style={{ fontFamily: R, fontSize: "11px", color: "#4A7C59", letterSpacing: "2px", marginBottom: "12px" }}>SUMMARY</div>
      ...
      {order.items && order.items.length > 0 && (
        <div style={{ background: "#FFFFFF", border: "2px solid #DDE8DD", borderRadius: "12px", padding: "20px" }}>
          <div style={{ fontFamily: R, fontSize: "11px", color: "#4A7C59", letterSpacing: "2px", marginBottom: "12px" }}>ITEMS</div>
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            {order.items.map((item: any, i: number) => (
              <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: i < order.items.length - 1 ? "1px solid #DDE8DD" : "none" }}>
                <div>
                  <div style={{ fontFamily: R, fontSize: "13px", color: "#1B3A2D", letterSpacing: "1px" }}>{item.product?.name ?? `Product #${item.product_id?.slice(0, 8)}`}</div>
      ...
      {/* Status update */}
      <div style={{ background: "#FFFFFF", border: "2px solid #DDE8DD", borderRadius: "12px", padding: "20px", display: "flex", flexDirection: "column", gap: "20px" }}>
        <div style={{ fontFamily: R, fontSize: "11px", color: "#4A7C59", letterSpacing: "2px" }}>UPDATE STATUS</div>

        <div>
          <div style={{ fontFamily: B, fontSize: "12px", color: "#5A7A60", letterSpacing: "1px", marginBottom: "8px" }}>ORDER STATUS</div>
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            {ORDER_STATUSES.map(s => (
              <button key={s} onClick={() => setOrderStatus(s)} style={{ fontFamily: R, fontSize: "11px", letterSpacing: "1px", padding: "6px 14px", borderRadius: "20px", border: `2px solid ${orderStatus === s ? OC[s] : "#DDE8DD"}`, background: orderStatus === s ? OC[s] + "30" : "transparent", color: orderStatus === s ? OC[s] : "#5A7A60", cursor: "pointer" }}>
                {s.toUpperCase()}
              </button>
            ))}
          </div>
        ...
        <div>
          <div style={{ fontFamily: B, fontSize: "12px", color: "#5A7A60", letterSpacing: "1px", marginBottom: "8px" }}>PAYMENT STATUS</div>
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            {PAYMENT_STATUSES.map(s => (
              <button key={s} onClick={() => setPaymentStatus(s)} style={{ fontFamily: R, fontSize: "11px", letterSpacing: "1px", padding: "6px 14px", borderRadius: "20px", border: `2px solid ${paymentStatus === s ? PC[s] : "#DDE8DD"}`, background: paymentStatus === s ? PC[s] + "30" : "transparent", color: paymentStatus === s ? PC[s] : "#5A7A60", cursor: "pointer" }}>
                {s.toUpperCase()}
              </button>
            ))}
          </div>
        ...
        <button onClick={handleSave} disabled={saving} style={{ alignSelf: "flex-start", position: "relative", display: "inline-block", cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.7 : 1 }}>
          ...
          <span style={{ position: "relative", display: "block", fontFamily: R, fontSize: "12px", background: "#1A8040", color: "#1B3A2D", padding: "10px 28px", border: "2px solid #1B3A2D", borderRadius: "6px", letterSpacing: "1.5px" }}>

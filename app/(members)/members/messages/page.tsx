    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "0", width: "100%" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginBottom: "16px" }}>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search conversations..."
          style={{ width: "100%", background: "#F2F7F2", border: "1.5px solid #DDE8DD", borderRadius: "20px", padding: "10px 16px", color: "#1B3A2D", fontFamily: B, fontSize: "13px", outline: "none", boxSizing: "border-box" }} />
            ...
            <div key={room.id}
              ...
              <div style={{ flex: 1, minWidth: 0 }} onClick={() => router.push(`/members/messages/${room.id}`)}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "3px" }}>
                  <span style={{ fontFamily: R, fontSize: "13px", color: room.unread_count > 0 ? "#1B3A2D" : "#5A7A60", letterSpacing: "0.5px" }}>{getRoomName(room)}</span>
                  <span style={{ fontFamily: B, fontSize: "11px", color: "#5A7A60", flexShrink: 0 }}>{room.last_message ? formatTime(room.last_message.created_at) : ""}</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontFamily: B, fontSize: "12px", color: room.unread_count > 0 ? "#4A7C59" : "#5A7A60", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "300px", fontWeight: room.unread_count > 0 ? 600 : 400 }}>
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: "#FFFFFF", border: "2px solid #DDE8DD", borderRadius: "16px", padding: "24px", width: "480px", maxWidth: "90vw", maxHeight: "80vh", overflowY: "auto", display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            {selected.length > 1 && (
              <input value={groupName} onChange={e => setGroupName(e.target.value)} placeholder="Group name (optional)"
                style={{ width: "100%", background: "#F2F7F2", border: "1.5px solid #DDE8DD", borderRadius: "6px", padding: "10px 14px", color: "#1B3A2D", fontFamily: B, fontSize: "13px", outline: "none", boxSizing: "border-box" }} />
            )}

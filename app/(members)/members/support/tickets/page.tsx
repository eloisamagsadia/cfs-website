    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "24px", maxWidth: "700px" }}>
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          {tickets.map(t => (
            <div key={t.id} style={{ background: "#1A2614", border: `2px solid ${expanded === t.id ? "#3CCE2A40" : "#2C4820"}`, borderRadius: "12px", overflow: "hidden" }}>
              {/* Header */}
              <div onClick={() => setExpanded(expanded === t.id ? null : t.id)}
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "8px", flexShrink: 0 }}>
                  {t.admin_notes && <span style={{ fontFamily: R, fontSize: "9px", color: "#3CCE2A", background: "#1A3D14", borderRadius: "20px", padding: "2px 8px" }}>REPLIED</span>}
                  ...
                  <span style={{ color: "#5A7A50", fontSize: "12px" }}>{expanded === t.id ? "▲" : "▼"}</span>
                </div>
              </div>

              {/* Thread */}
              {expanded === t.id && (
                <div style={{ borderTop: "1px solid #2C4820", padding: "16px 20px", display: "flex", flexDirection: "column", gap: "12px" }}>
                  {/* Original message */}
                  <div style={{ background: "#243520", borderRadius: "8px", padding: "12px 14px" }}>
                    <div style={{ fontFamily: R, fontSize: "10px", color: "#5A7A50", letterSpacing: "1px", marginBottom: "6px" }}>YOUR MESSAGE</div>
                    <div style={{ fontFamily: B, fontSize: "13px", color: "#F0EAD6", lineHeight: 1.6 }}>{t.message}</div>
                  </div>
                      ...
                      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                        {t.attachments.map((url: string, i: number) => (
                          <a key={i} href={url} target="_blank" rel="noopener noreferrer">
                            <img src={url} alt="" style={{ width: "80px", height: "80px", objectFit: "cover", borderRadius: "8px", border: "1.5px solid #2C4820", cursor: "pointer" }} />
                          </a>
                        ))}
                      </div>
                    </div>
                  ...
                  {t.admin_notes && t.status !== "closed" && (
                    <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                      <textarea
                        ...
                        rows={3}
                        style={{ width: "100%", background: "#243520", border: "1.5px solid #2C4820", borderRadius: "6px", padding: "10px 14px", color: "#F0EAD6", fontFamily: B, fontSize: "13px", outline: "none", resize: "none", boxSizing: "border-box" }}
                      />
                      <button onClick={() => handleReply(t.id)} disabled={replying === t.id || !replyText[t.id]?.trim()}
                        style={{ alignSelf: "flex-end", fontFamily: R, fontSize: "11px", background: replying === t.id || !replyText[t.id]?.trim() ? "#243520" : "#3CCE2A", color: replying === t.id || !replyText[t.id]?.trim() ? "#5A7A50" : "#080F06", border: "none", borderRadius: "6px", padding: "8px 20px", cursor: "pointer", letterSpacing: "1px" }}>

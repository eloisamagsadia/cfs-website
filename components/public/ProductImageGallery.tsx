        ...
        <div
          ...
          {!inStock && (
            <div style={{ position: "absolute", inset: 0, background: "rgba(8,15,6,0.7)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <span style={{ fontFamily: R, fontSize: "16px", color: "#CC3344", letterSpacing: "2px", background: "#FFE8EC", border: "2px solid #CC3344", borderRadius: "8px", padding: "10px 20px" }}>OUT OF STOCK</span>
            </div>
          )}
        </div>
      </div>
        ...
        <div style={{ display: "flex", gap: "8px", maxWidth: "100%" }}>
          {images.slice(0, 4).map((img, i) => (
            <div key={i} onClick={() => { setSelected(i); }}
              style={{ width: "72px", height: "72px", borderRadius: "8px", overflow: "hidden", border: `2px solid ${selected === i ? accentColor : "#DDE8DD"}`, flexShrink: 0, cursor: "pointer", transition: "border-color 0.15s", opacity: selected === i ? 1 : 0.6 }}>
          ...
          {images.length > 4 && (
            <div onClick={() => { setSelected(4); setLightbox(true); }}
              style={{ width: "72px", height: "72px", borderRadius: "8px", overflow: "hidden", border: "2px solid #DDE8DD", flexShrink: 0, cursor: "pointer", background: "#FFFFFF", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: "2px" }}>
              <span style={{ fontFamily: "var(--font-righteous,'Righteous',sans-serif)", fontSize: "14px", color: "#1B3A2D" }}>+{images.length - 4}</span>
              <span style={{ fontFamily: "var(--font-barlow,'Barlow',sans-serif)", fontSize: "9px", color: "#5A7A60", letterSpacing: "1px" }}>MORE</span>
            </div>
          )}
        </div>
      )}
      ...
      {lightbox && (
        <div onClick={() => { setLightbox(false); setZoomed(false); }}
          ...
          {selected < images.length - 1 && (
            <button onClick={e => { e.stopPropagation(); setSelected(s => s + 1); }}
              style={{ position: "absolute", right: "20px", background: "rgba(255,255,255,0.1)", border: "none", color: "#1B3A2D", fontSize: "28px", cursor: "pointer", borderRadius: "50%", width: "48px", height: "48px", zIndex: 2001 }}>›</button>
          )}
            ...
            <div style={{ position: "absolute", bottom: "20px", left: "50%", transform: "translateX(-50%)", display: "flex", gap: "8px" }}>
              {images.map((_, i) => (
                <button key={i} onClick={e => { e.stopPropagation(); setSelected(i); }}
                  style={{ width: "8px", height: "8px", borderRadius: "50%", background: selected === i ? "#1B3A2D" : "rgba(255,255,255,0.3)", border: "none", cursor: "pointer", padding: 0 }} />
              ))}
            </div>
          )}
        </div>

  ...
  return (
    <div>
      ...
      <div style={{ position: "relative", padding: "4px 4px 8px 0", marginBottom: "10px" }}>
        ...
        <div
          onMouseEnter={() => !lightbox && setZoomed(true)}
          ...
          style={{ position: "relative", background: "#1A2614", border: "2px solid #2C4820", borderRadius: "12px", overflow: "hidden", zIndex: 1, height: "420px", cursor: "zoom-in" }}>
          {images[selected]
        ...
        <div style={{ display: "flex", gap: "8px", overflowX: "auto" }}>
          {images.map((img, i) => (
            <div key={i} onClick={() => setSelected(i)}
              style={{ width: "80px", height: "80px", borderRadius: "8px", overflow: "hidden", border: `2px solid ${selected === i ? accentColor : "#2C4820"}`, flexShrink: 0, cursor: "pointer", transition: "border-color 0.15s", opacity: selected === i ? 1 : 0.6 }}>
    ...
    {/* Lightbox */}
      {lightbox && (
        <div onClick={() => { setLightbox(false); setZoomed(false); }}
          style={{ position: "fixed", inset: 0, zIndex: 2000, background: "rgba(0,0,0,0.95)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "zoom-out" }}>

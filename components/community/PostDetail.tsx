        ...
        {myReaction ? (
          ...
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
            <path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"
              stroke="#5A7A50" strokeWidth="1.8" fill="none"/>
          </svg>
        )}
      </button>
      {showPicker && (
        <div style={{ position:"absolute", bottom:"calc(100% + 6px)", left:0, background:"#1A2614", border:"2px solid #2C4820", borderRadius:"14px", padding:"6px 8px", display:"flex", gap:"4px", zIndex:30, boxShadow:"0 4px 20px rgba(0,0,0,0.5)", animation:"fadeUp 0.15s ease" }}>
          {COMMENT_REACTIONS.map(({emoji,label}) => (
            <button key={emoji} onClick={() => handlePick(emoji)} title={label}
              style={{ background:myReaction===emoji?"#1A3D14":"none", border:myReaction===emoji?"1.5px solid #3CCE2A":"1.5px solid transparent", borderRadius:"8px", padding:"5px 7px", cursor:"pointer", fontSize:"18px", transition:"all 0.15s" }}
...
const ReplyInput = memo(function ReplyInput({ commentId, authorName, currentUser, onSubmit, onCancel }: {
  ...
  return (
    <div style={{ marginLeft:"46px", marginTop:"8px", display:"flex", gap:"8px", alignItems:"flex-end" }}>
      ...
      <div style={{ flex:1, background:"#243520", border:"1.5px solid #3CCE2A", borderRadius:"12px", padding:"8px 14px" }}>
        <textarea
          ref={ref}
          value={text}
          ...
          rows={1}
          style={{ width:"100%", background:"transparent", border:"none", outline:"none", color:"#F0EAD6", fontFamily:B, fontSize:"13px", resize:"none", lineHeight:1.5, maxHeight:"80px", overflow:"auto" }}
        />
        {text.trim() && (
      ...
      <button onClick={onCancel} style={{ background:"none", border:"none", color:"#5A7A50", cursor:"pointer", fontSize:"16px", paddingBottom:"4px" }}>✕</button>
    </div>
  );
});
      ...
      <div style={{ display:"flex", gap:"10px", alignItems:"flex-start" }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ padding:"2px 0 6px" }}>
            ...
            <div style={{ fontFamily:B, fontSize:"13px", color:"#C8C0A8", lineHeight:1.65, wordBreak:"break-word" }}
          ...
          <div style={{ display:"flex", gap:"12px", alignItems:"center", paddingLeft:"4px" }}>
            ...
            {level===0 && <button onClick={()=>onReply(comment.id===replyingToId?null:comment.id,cp.display_name??"Member")} style={{ background:"none", border:"none", cursor:"pointer", fontFamily:B, fontSize:"11px", color:comment.id===replyingToId?"#3CCE2A":"#5A7A50", padding:0 }}>
  ...
  return (
    <div style={{ maxWidth:"680px", margin:"0 auto" }}>
      ...
      <div style={{ background:"#1A2614", border:"2px solid #2C4820", borderRadius:"16px", overflow:"hidden" }}>
        ...
        {/* Analytics */}
        <div style={{ margin:"8px 18px", borderTop:"1px solid #2C4820", paddingTop:"10px" }}>
        ...
        <div style={{ borderTop:"1px solid #2C4820", margin:"0 18px" }}/>

        {/* Comments */}
        <div style={{ padding:"14px 18px" }}>
          {topComments.length>0&&<div style={{ fontFamily:B, fontSize:"12px", color:"#5A7A50", marginBottom:"14px" }}>{comments.length} comment{comments.length!==1?"s":""}</div>}
      ...
      {showMenu && menuPos && createPortal(
        <>
          ...
          <div style={{ position:"fixed", top:menuPos.top, right:menuPos.right, background:"#1A2614", border:"2px solid #2C4820", borderRadius:"12px", padding:"6px", zIndex:999, minWidth:"170px", boxShadow:"0 8px 32px rgba(0,0,0,0.5)" }}>
            <button onClick={handleCopyLink}
              style={{ width:"100%", display:"flex", alignItems:"center", gap:"10px", padding:"9px 12px", borderRadius:"8px", background:"none", border:"none", cursor:"pointer", fontFamily:B, fontSize:"13px", color:"#F0EAD6", textAlign:"left" }}

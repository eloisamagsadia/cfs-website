...
export default function MobileSuperNav() {
  const pathname = usePathname();

  return (
    <nav style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 50, background: "rgba(247,250,245,0.96)", borderTop: "1px solid #F0DFA0", padding: "8px 0 calc(8px + env(safe-area-inset-bottom))", display: "flex", justifyContent: "space-around", alignItems: "center" }}>
      {tabs.map(({ label, href, icon, exact }) => {
        const isActive = exact ? pathname === href : pathname.startsWith(href);
        return (
          <Link key={href} href={href} style={{ textDecoration: "none", display: "flex", flexDirection: "column", alignItems: "center", gap: "3px", padding: "4px 10px", borderRadius: "10px", background: isActive ? "#E8F4EC" : "transparent", transition: "background 0.15s" }}>
            <span style={{ color: isActive ? "#156530" : "#5A7A60" }}>{icon}</span>
            <span style={{ fontFamily: R, fontSize: "9px", color: isActive ? "#156530" : "#5A7A60", letterSpacing: "1px" }}>{label.toUpperCase()}</span>
          </Link>
        );
      })}
    </nav>
